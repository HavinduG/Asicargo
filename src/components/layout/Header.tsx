    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                {/* Logo */}
                <Link href="/" className="relative z-10 block" onClick={() => setIsMobileMenuOpen(false)}>
                    <Image
                        src={header_logo.url}
            ...
            {/* Mobile Menu Overlay */}
            <div
                className={`md:hidden absolute top-full left-0 right-0 bg-white dark:bg-zinc-950 transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? "max-h-[calc(100vh-5rem)] opacity-100 py-4 shadow-lg border-b border-zinc-200 dark:border-zinc-800" : "max-h-0 opacity-0 py-0 border-transparent"
                    }`}
            >
